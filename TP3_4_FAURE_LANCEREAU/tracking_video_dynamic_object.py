...
###Step3
def initialize_particles(N, center_x, center_y, size_range):
    particles = []
    for _ in range(N):
        x = random.randint(center_x - size_range[0]//2, center_x + size_range[0]//2)
        y = random.randint(center_y - size_range[1]//2, center_y + size_range[1]//2)
        size = random.uniform(size_range[0], size_range[1])
        particle = [x, y, size, 1/N]
        particles.append(particle)
    return particles

# Example initialization with size range (min_size, max_size)
init_particles = initialize_particles(100, center_x, center_y, (min_size, max_size))
